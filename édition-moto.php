...
if(isset($_GET['numchassis'])){
	...
	$reqmoto=$pdo->prepare("SELECT * FROM moto WHERE num_chassis = ?");
	$reqmoto->execute(array($getnum_chassis ));
	$motoinfo=$reqmoto->fetch();
/* modification de la marque*/
  if(isset($_POST['nvmarque']) and !empty($_POST['nvmarque'] ) and $_POST['nvmarque'] != $motoinfo['marque']){
    $nvmarque=htmlspecialchars($_POST['nvmarque']);
    $update=$pdo->prepare("UPDATE  moto SET marque =? WHERE num_chassis = ?");
...
/* modification de nom */
if(isset($_POST['nvnom']) and !empty($_POST['nvnom'] ) and $_POST['nvnom'] != $motoinfo['nom_moto']){
  $nvnom=htmlspecialchars($_POST['nvnom']);
  $update=$pdo->prepare("UPDATE moto SET nom_moto =? WHERE num_chassis = ?");
  $update->execute(array($nvnom,$getnum_chassis ));
  header("Location:../mes-déclarations.php?msg2=déclaration-modifiée&id=".$_SESSION['id']);  
 
}
/* modification de matricule*/
if(isset($_POST['nvmatricule']) and !empty($_POST['nvmatricule'] ) and $_POST['nvmatricule'] != $motoinfo['matricule']){
  $nvmatricule=htmlspecialchars($_POST['nvmatricule']);
  $update=$pdo->prepare("UPDATE moto SET matricule=? WHERE num_chassis = ?");
  $update->execute(array($nvmatricule,$getnum_chassis ));
  header("Location:../mes-déclarations.php?msg2=déclaration-modifiée&id=".$_SESSION['id']);  
 
}
/* modification de num chassis*/
if(isset($_POST['nvnumchassis']) and !empty($_POST['nvnumchassis'] ) and sha1($_POST['nvnumchassis'])!= $motoinfo['num_chassis ']){
  ...
  $reqnvnumchassisi=$pdo->prepare("SELECT * FROM moto WHERE  num_chassis =?");
  $reqnvnumchassisi->execute(array($nvnumchassis));
 $nvnumchassisiexist= $reqnvnumchassisi->rowCount();
 if($nvnumchassisiexist==0){
$req= $pdo->prepare("UPDATE  moto SET num_chassis =? WHERE num_s=? ");
$req->execute(array($nvnumchassis,$motoinfo['num_chassis ']));
...
if(isset($_POST['nvclr']) and !empty($_POST['nvclr'] ) and $_POST['nvclr'] != $motoinfo['couleur']){
  $nvclr=htmlspecialchars($_POST['nvclr']);
  $update=$pdo->prepare("UPDATE moto SET couleur=? WHERE num_chassis= ?");
...
?>
      ...
      <h2>modifier votre déclaration de moto <script src="https://cdn.lordicon.com/libs/mssddfmo/lord-icon-2.1.0.js"></script>
        <lord-icon
   ...
   <form name="formaadd" method="post" action="" enctype="multipart/form-data">
      ...
      <input type="text" name="nvmarque" id="nvmarque" placeholder="<?php echo $motoinfo['marque'] ?>">
    </div>
    <div class="inputbx"data-center-center="opacity:1;left:0;"data-0-bottom="opacity:0;left:-50px;">
      <label>Nom de la moto</label>
      <input type="text" name="nvnom" id="nvnom" placeholder="<?php echo $motoinfo['nom_moto'] ?>">
    </div>
    <div class="inputbx"data-center-center="opacity:1;left:0;"data-0-bottom="opacity:0;left:-50px;">
    ...
    <div class="inputbx"data-center-center="opacity:1;left:0;"data-0-bottom="opacity:0;left:-50px;">
      ...
      <input type="text" name="nvmatricule" id="nvmatricule" placeholder="<?php echo $motoinfo['matricule'] ?>">
    </div>
    <div class="inputbx"data-center-center="opacity:1;left:0;"data-0-bottom="opacity:0;left:-50px;">
      <label>Couleur de moto</label>
      <input type="text" name="nvclr" id="nvclr" placeholder="<?php echo $motoinfo['couleur'] ?> ">
    </div>
            <br>
