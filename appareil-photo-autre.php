...
$justificatifs=$_FILES['just']['name'];
   ...
   and!empty($_POST['clr'])
    ...
    if($imageFileType != "jpg" && $imageFileType != "png" && $imageFileType != "jpeg"){
      ...
      $reqnums=$pdo->prepare("SELECT * FROM appareil_photo WHERE  num_s=?");
      $reqnums->execute(array($num_s));
     $numsexist= $reqnums->rowCount();
     if($numsexist==0){
      $req= $pdo->prepare("INSERT INTO appareil_photo(id,marque,nom_appareil,num_s,couleur,photo,justificatif,confirmation) VALUES (?,?,?,?,?,?,?,?)");
      $req->execute(array($ID,$marque,$nom_appareil,$num_s,$couleur,$_FILES['image']['name'],$_FILES['just']['name'],''));
	if($req){
move_uploaded_file($_FILES["image"]["tmp_name"],"../appareils-photo-déclarés/$img");
move_uploaded_file($_FILES["just"]["tmp_name"],"../justificatifs -appareil-photo/$justificatifs");
...
<html>
   <head>
      ...
      <title>Déclaration d'appareil-photo</title>  
...
<body>
  ...
  <section class="banner">
    <header>
      <a href="appareil-photo.php" class="re" ><i id ="icon"class="fas fa-arrow-alt-circle-left"></i> Retour</a>
     <div class="alerte">
...
?>
      ...
      <h2>Remplissez les informations de votre appareil photo <script src="https://cdn.lordicon.com/libs/mssddfmo/lord-icon-2.1.0.js"></script>
        <lord-icon
