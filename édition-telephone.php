...
/* modification de la marque*/
  if(isset($_POST['nvmarque']) and !empty($_POST['nvmarque'] ) and $_POST['nvmarque'] != $telinfo['marque']){
    $nvmarque=htmlspecialchars($_POST['nvmarque']);
    $update=$pdo->prepare("UPDATE  telephone SET marque =? WHERE emei= ?");
    $update->execute(array($nvmarque,$getemei));
    header("Location:../mes-déclarations.php?msg2=déclaration-modifiée&id=".$_SESSION['id']);  
   
}
/* modification de nom */
if(isset($_POST['nvnom']) and !empty($_POST['nvnom'] ) and $_POST['nvnom'] != $telinfo['nom_appareil']){
  $nvnom=htmlspecialchars($_POST['nvnom']);
  $update=$pdo->prepare("UPDATE telephone SET nom_appareil=? WHERE emei= ?");
  $update->execute(array($nvnom,$getemei));
  header("Location:../mes-déclarations.php?msg2=déclaration-modifiée&id=".$_SESSION['id']);  
 
}
/* modification de num serie */
if(isset($_POST['nvnums']) and !empty($_POST['nvnums'] ) and $_POST['nvnums'] != $telinfo['num_s']){
  $nvnums=htmlspecialchars($_POST['nvnums']);
  $update=$pdo->prepare("UPDATE telephone SET num_s=? WHERE emei= ?");
  $update->execute(array($nvnums,$getemei));
  header("Location:../mes-déclarations.php?msg2=déclaration-modifiée&id=".$_SESSION['id']);  
 
}
/* modification de 'emei */
if(isset($_POST['nvxemei']) and !empty($_POST['nvxemei'] ) and sha1($_POST['nvxemei'])!= $telinfo['emei']){
  ...
  $reqemei=$pdo->prepare("SELECT * FROM telephone WHERE  emei=?");
  $reqemei->execute(array($nvxemei));
 $emeiexist= $reqemei->rowCount();
 if($emeiexist==0){
$req= $pdo->prepare("UPDATE  telephone SET emei=? WHERE num_s=? ");
$req->execute(array($nvxemei,$telinfo['num_s']));
...
if(isset($_POST['nvclr']) and !empty($_POST['nvclr'] ) and $_POST['nvclr'] != $telinfo['couleur']){
  $nvclr=htmlspecialchars($_POST['nvclr']);
  $update=$pdo->prepare("UPDATE telephone SET couleur=? WHERE emei= ?");
...
?>
      ...
      <h2>modifier votre déclaration du téléphone<script src="https://cdn.lordicon.com/libs/mssddfmo/lord-icon-2.1.0.js"></script>
        <lord-icon
   ...
   <form name="formaadd" method="post" action="" enctype="multipart/form-data">
      ...
      <input type="text" name="nvmarque" id="nvmarque" placeholder="<?php echo $telinfo['marque'];?>">
    </div>
    <div class="inputbx"data-center-center="opacity:1;left:0;"data-0-bottom="opacity:0;left:-50px;">
      <label>Nom de l'appareil</label>
      <input type="text" name="nvnom" id="nvnom" placeholder="<?php echo $telinfo['nom_appareil'];?>">
    </div>
    <div class="inputbx"data-center-center="opacity:1;left:0;"data-0-bottom="opacity:0;left:-50px;">
      <label> numéro de série</label> 
      <input type="text" name="nvnums" id="nvnums" placeholder="<?php echo $telinfo['num_s'];?>">
    </div>
    <div class="inputbx"data-center-center="opacity:1;left:0;"data-0-bottom="opacity:0;left:-50px;">
      <label>Imei</label>
    ...
    <div class="inputbx"data-center-center="opacity:1;left:0;"data-0-bottom="opacity:0;left:-50px;">
      ...
      <input type="text" name="nvclr" id="nvclr" placeholder="<?php echo $telinfo['couleur'];?>">
    </div>
            <br>
