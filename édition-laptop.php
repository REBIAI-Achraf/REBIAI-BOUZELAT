...
if(isset($_GET['nums'])){
	$getnum_s=$_GET['nums'];
	$reqlap=$pdo->prepare("SELECT * FROM laptop WHERE num_s= ?");
	$reqlap->execute(array($getnum_s));
	$lapinfo=$reqlap->fetch();
/* modification de la marque*/
  if(isset($_POST['nvmarque']) and !empty($_POST['nvmarque'] ) and $_POST['nvmarque'] != $lapinfo['marque']){
    $nvmarque=htmlspecialchars($_POST['nvmarque']);
    $update=$pdo->prepare("UPDATE  laptop SET marque =? WHERE num_s= ?");
    $update->execute(array($nvmarque,$getnum_s));
    header("Location:../mes-déclarations.php?msg2=déclaration-modifiée&id=".$_SESSION['id']);  
   
}
/* modification de nom */
if(isset($_POST['nvnom']) and !empty($_POST['nvnom'] ) and $_POST['nvnom'] != $lapinfo['nom_appareil']){
  $nvnom=htmlspecialchars($_POST['nvnom']);
  $update=$pdo->prepare("UPDATE laptop SET nom_appareil=? WHERE num_s= ?");
  $update->execute(array($nvnom,$getnum_s));
  header("Location:../mes-déclarations.php?msg2=déclaration-modifiée&id=".$_SESSION['id']);  
 
}
/* modification de id modele*/
if(isset($_POST['nvidm']) and !empty($_POST['nvidm'] ) and $_POST['nvidm'] != $lapinfo['id_modele']){
  $nvnums=htmlspecialchars($_POST['nvidm']);
  $update=$pdo->prepare("UPDATE laptop SET id_modele=? WHERE num_s= ?");
  $update->execute(array($nvnums,$getnum_s));
  header("Location:../mes-déclarations.php?msg2=déclaration-modifiée&id=".$_SESSION['id']);  
 
}
/* modification de num serie */
if(isset($_POST['nvnums']) and !empty($_POST['nvnums'] ) and sha1($_POST['nvnums'])!= $lapinfo['num_s']){
  $nvnums=sha1($_POST['nvnums']);
  $num_slenth=strlen($_POST['nvnums']);
  if($num_slenth < 10 or $num_slenth > 17 ){
    $erreur="le numéro de serie que que vous avez rentré est invalide";
}else{
  $reqnum_s=$pdo->prepare("SELECT * FROM laptop WHERE  num_s=?");
  $reqnum_s->execute(array($nvnums));
 $num_sexist= $reqnum_s->rowCount();
 if($num_sexist==0){
$req= $pdo->prepare("UPDATE  laptop SET num_s=? WHERE num_s=? ");
$req->execute(array($nvnums,$lapinfo['num_s']));
...
if(isset($_POST['nvclr']) and !empty($_POST['nvclr'] ) and $_POST['nvclr'] != $lapinfo['couleur']){
  $nvclr=htmlspecialchars($_POST['nvclr']);
  $update=$pdo->prepare("UPDATE laptop SET couleur=? WHERE num_s= ?");
...
?>
      ...
      <h2>modifier votre déclaration du Laptop<script src="https://cdn.lordicon.com/libs/mssddfmo/lord-icon-2.1.0.js"></script>
        <lord-icon
   ...
   <form name="formaadd" method="post" action="" enctype="multipart/form-data">
      ...
      <input type="text" name="nvmarque" id="marque" placeholder="<?php echo $lapinfo['marque'];?>">
    </div>
    <div class="inputbx">
      <label>Nom / modèle de l'appareil</label>
      <input type="text" name="nvnom" id="nom" placeholder="<?php echo $lapinfo['nom_appareil'];?>">
    </div>
    <div class="inputbx">
    ...
    <div class="inputbx">
      ...
      <input type="text" name="nvidm" id="idm" placeholder="<?php echo $lapinfo['id_modele'];?>">
    </div>
    <div class="inputbx">
      <label>Couleur de l'appareil</label>
      <input type="text" name="nvclr" id="clr" placeholder="<?php echo $lapinfo['couleur'];?>">
    </div>
            <br>
