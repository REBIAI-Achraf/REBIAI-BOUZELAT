...
$justificatifs=$_FILES['just']['name'];
   ...
   and!empty($_POST['taille'])
    ...
    if($imageFileType != "jpg" && $imageFileType != "png" && $imageFileType != "jpeg"){
      ...
      $reqnums=$pdo->prepare("SELECT * FROM tv WHERE  num_s=?");
      $reqnums->execute(array($num_s));
     $numsexist= $reqnums->rowCount();
     if($numsexist==0){
	$req= $pdo->prepare("INSERT INTO tv(id,marque,typeTV,num_s,couleur,taille,photo,justificatif,confirmation) VALUES (?,?,?,?,?,?,?,?,?)");
	$req->execute(array($ID,$marque,$nom_appareil,$num_s,$couleur,$taille,$_FILES['image']['name'],$_FILES['just']['name'],''));
	if($req){
move_uploaded_file($_FILES["image"]["tmp_name"],"../TV-déclarés/$img");
move_uploaded_file($_FILES["just"]["tmp_name"],"../justificatifs -TV/$justificatifs");
...
<body>
  ...
  <section class="banner">
    <header>
      <a href="TV.php" class="re" ><i id ="icon"class="fas fa-arrow-alt-circle-left"></i> Retour</a>
     <div class="alerte">
...
?>
      ...
      <h2>Remplissez les informations de votre TV <script src="https://cdn.lordicon.com/libs/mssddfmo/lord-icon-2.1.0.js"></script>
        <lord-icon
   ...
   <form name="formaadd" method="post" action="" enctype="multipart/form-data">
    ...
    <div class="inputbx">
      <label>Type / modèle de l'appareil</label>
      <input type="text" name="nom" id="nom" placeholder="Smart TV , TV Normal...">
    </div>
    <div class="inputbx">
    ...
    <div class="inputbx">
      <label> Taille du TV </label>
      <input type="text" name="taille" id="taille" placeholder="Entrer la taille en pouce">
    </div>
    <div class="inputbx">
