...
{   $marque=$_POST['marque'];
	...
	$couleur=$_POST['clr'];
  $imeilenth=strlen($_POST['imei']);
  $imei=sha1($_POST['imei']);
...
$justificatifs=$_FILES['just']['name'];
	...
	if(!empty($_POST['nom'])
   and!empty($_POST['imei'])
   and!empty($_POST['clr'])
   ){	
    if($imeilenth <15 AND $imeilenth >18){
        $erreur="l'imei de série que vous avez rentré est invalide";
    }
    ...
    if($imageFileType != "jpg" && $imageFileType != "png" && $imageFileType != "jpeg"){
      ...
      $reqimei->execute(array($imei));
     $imeiexist= $reqimei->rowCount();
     if($imeiexist==0){
      $req= $pdo->prepare("INSERT INTO smart_watch(id,marque,nom_appareil,imei,couleur,photo,justificatif,confirmation) VALUES (?,?,?,?,?,?,?,?)");
      $req->execute(array($ID,$marque,$nom_appareil,$imei,$couleur,$_FILES['image']['name'],$_FILES['just']['name'],''));
	if($req){
move_uploaded_file($_FILES["image"]["tmp_name"],"../SmartWatch-déclarés/$img");
move_uploaded_file($_FILES["just"]["tmp_name"],"../justificatifs-SmartWatch/$justificatifs");
    $erreur2="votre déclaration a été envoyée !";
  }
    }
     else{
      $erreur="appareil déja déclaré ,imei déja existe !";
     }  
  }
...
<body>
  ...
  <section class="banner">
    <header>
      <a href="SmartWatch.php" class="re" ><i id ="icon"class="fas fa-arrow-alt-circle-left"></i> Retour</a>
     <div class="alerte">
...
?>
      ...
      <h2>Remplissez les informations de votre Smart Watch <script src="https://cdn.lordicon.com/libs/mssddfmo/lord-icon-2.1.0.js"></script>
        <lord-icon
   ...
   <form name="formaadd" method="post" action="" enctype="multipart/form-data">
    ...
    <div class="inputbx">
      <label>Nom / modèle de Smart Watch</label>
      <input type="text" name="nom" id="nom" placeholder="Entrer le nom de l'apareil">
    </div>
    <div class="inputbx">
      <label> IMEI </label> 
      <input type="text" name="imei" id="imei" placeholder="Entrer le numéro de série">
    </div>
    <div class="inputbx">
      <label>Couleur de Smart Watch</label>
