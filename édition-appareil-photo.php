...
if(isset($_GET['nums'])){
	$getnum_s=$_GET['nums'];
	$reqapp=$pdo->prepare("SELECT * FROM appareil_photo WHERE num_s= ?");
	$reqapp->execute(array($getnum_s));
	$appinfo=$reqapp->fetch();
/* modification de la marque*/
  if(isset($_POST['nvmarque']) and !empty($_POST['nvmarque'] ) and $_POST['nvmarque'] != $appinfo['marque']){
    $nvmarque=htmlspecialchars($_POST['nvmarque']);
    $update=$pdo->prepare("UPDATE  appareil_photo SET marque =? WHERE num_s= ?");
    $update->execute(array($nvmarque,$getnum_s));
    header("Location:../mes-déclarations.php?msg2=déclaration-modifiée&id=".$_SESSION['id']);  
   
}
/* modification de nom */
if(isset($_POST['nvnom']) and !empty($_POST['nvnom'] ) and $_POST['nvnom'] != $appinfo['nom_appareil']){
  $nvnom=htmlspecialchars($_POST['nvnom']);
  $update=$pdo->prepare("UPDATE appareil_photo SET nom_appareil=? WHERE num_s= ?");
...
if(isset($_POST['nvnums']) and !empty($_POST['nvnums'] ) and sha1($_POST['nvnums'])!= $appinfo['num_s']){
  $nvnums=sha1($_POST['nvnums']);
  $num_slenth=strlen($_POST['nvnums']);
  if($num_slenth > 25 ){
    $erreur="le numéro de serie que vous avez rentré est invalide";
}else{
  $reqnum_s=$pdo->prepare("SELECT * FROM appareil_photo WHERE  num_s=?");
  $reqnum_s->execute(array($nvnums));
 $num_sexist= $reqnum_s->rowCount();
 if($num_sexist==0){
$req= $pdo->prepare("UPDATE  appareil_photo SET num_s=? WHERE num_s=? ");
$req->execute(array($nvnums,$appinfo['num_s']));
...
if(isset($_POST['nvclr']) and !empty($_POST['nvclr'] ) and $_POST['nvclr'] != $appinfo['couleur']){
  $nvclr=htmlspecialchars($_POST['nvclr']);
  $update=$pdo->prepare("UPDATE appareil_photo SET couleur=? WHERE num_s= ?");
...
?>
      ...
      <h2>modifier votre déclaration d'appareil photo <script src="https://cdn.lordicon.com/libs/mssddfmo/lord-icon-2.1.0.js"></script>
        <lord-icon
   ...
   <form name="formaadd" method="post" action="" enctype="multipart/form-data">
      ...
      <input type="text" name="nvmarque" id="marque" placeholder="<?php echo $appinfo['marque'];?>">
    </div>
    <div class="inputbx">
      <label>Nom / modèle de l'appareil</label>
      <input type="text" name="nvnom" id="nom" placeholder="<?php echo $appinfo['nom_appareil'];?>">
    </div>
    <div class="inputbx">
    ...
    <div class="inputbx">
      ...
      <input type="text" name="nvclr" id="clr" placeholder="<?php echo $appinfo['couleur'];?>">
    </div>
            <br>
