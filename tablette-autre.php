...
{   $marque=$_POST['marque'];
  $nom_appareil=htmlspecialchars($_POST['nom']);
	$num_s=$_POST['nums'];
	$couleur=$_POST['clr'];
    $emeilenth=strlen($_POST['xemei']);
  $emei=sha1($_POST['xemei']);
...
$justificatifs=$_FILES['just']['name'];
   ...
   and!empty($_POST['clr'])
   and!empty($_POST['xemei'])
   ){	
    if($emeilenth < 15 or $emeilenth > 18 ){
        $erreur="l'Emei que vous avez rentré est invalide";
    }
    ...
    if($imageFileType != "jpg" && $imageFileType != "png" && $imageFileType != "jpeg"){
      ...
      $reqemei->execute(array($emei));
     $emeiexist= $reqemei->rowCount();
     if($emeiexist==0){
      $req= $pdo->prepare("INSERT INTO tablette(id,marque,nom_appareil,num_s,couleur,emei,photo,justificatif,confirmation) VALUES (?,?,?,?,?,?,?,?,?)");
      $req->execute(array($ID,$marque,$nom_appareil,$num_s,$couleur,$emei,$_FILES['image']['name'],$_FILES['just']['name'],''));
	if($req){
move_uploaded_file($_FILES["image"]["tmp_name"],"../tablettes-déclarés/$img");
move_uploaded_file($_FILES["just"]["tmp_name"],"../justificatifs-tab/$justificatifs");
    $erreur2="votre déclaration a été envoyée !";
  }
    }
     else{
      $erreur="appareil déja déclaré ,Imei existe déja !";
     }  
  }
...
<body>
  ...
  <section class="banner">
    <header>
      <a href="tablettes.php" class="re" ><i id ="icon"class="fas fa-arrow-alt-circle-left"></i> Retour</a>
     <div class="alerte">
...
?>
      ...
      <h2>Remplissez les informations de votre tablette <script src="https://cdn.lordicon.com/libs/mssddfmo/lord-icon-2.1.0.js"></script>
        <lord-icon
   ...
   <form name="formaadd" method="post" action="" enctype="multipart/form-data">
    ...
    <div class="inputbx"data-center-center="opacity:1;left:0;"data-0-bottom="opacity:0;left:-50px;">
      <label>Imei</label>
      <input type="text" name="xemei" id="xemei" placeholder="Entrer l'IMEI">
    </div>
    <div class="inputbx"data-center-center="opacity:1;left:0;"data-0-bottom="opacity:0;left:-50px;">
