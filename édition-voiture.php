...
if(isset($_GET['numchassis'])){
	$getnum_chassis=$_GET['numchassis'];
	$reqvoiture=$pdo->prepare("SELECT * FROM voiture WHERE num_chassis = ?");
	$reqvoiture->execute(array($getnum_chassis ));
	$voitureinfo=$reqvoiture->fetch();
/* modification de la marque*/
  if(isset($_POST['nvmarque']) and !empty($_POST['nvmarque'] ) and $_POST['nvmarque'] != $voitureinfo['marque']){
    $nvmarque=htmlspecialchars($_POST['nvmarque']);
    $update=$pdo->prepare("UPDATE  voiture SET marque =? WHERE num_chassis = ?");
    $update->execute(array($nvmarque,$getnum_chassis ));
    header("Location:../mes-déclarations.php?msg2=déclaration-modifiée&id=".$_SESSION['id']);  
   
}
/* modification de nom */
if(isset($_POST['nvnom']) and !empty($_POST['nvnom'] ) and $_POST['nvnom'] != $voitureinfo['nom_voiture']){
  $nvnom=htmlspecialchars($_POST['nvnom']);
  $update=$pdo->prepare("UPDATE voiture SET nom_voiture =? WHERE num_chassis = ?");
  $update->execute(array($nvnom,$getnum_chassis ));
  header("Location:../mes-déclarations.php?msg2=déclaration-modifiée&id=".$_SESSION['id']);  
 
}
/* modification de matricule*/
if(isset($_POST['nvmatricule']) and !empty($_POST['nvmatricule'] ) and $_POST['nvmatricule'] != $voitureinfo['matricule']){
  $nvmatricule=htmlspecialchars($_POST['nvmatricule']);
  $update=$pdo->prepare("UPDATE voiture SET matricule=? WHERE num_chassis = ?");
  $update->execute(array($nvmatricule,$getnum_chassis ));
  header("Location:../mes-déclarations.php?msg2=déclaration-modifiée&id=".$_SESSION['id']);  
 
}
/* modification de num chassis*/
if(isset($_POST['nvnumchassis']) and !empty($_POST['nvnumchassis'] ) and sha1($_POST['nvnumchassis'])!= $voitureinfo['num_chassis ']){
  $nvnumchassis=sha1($_POST['nvnumchassis']);
  $nvnumchassisilenth=strlen($_POST['nvnumchassis']);
  if( $nvnumchassisilenth > 17 ){
    $erreur="le numéro de chassis que vous avez rentré est invalide";
}else{
  $reqnvnumchassisi=$pdo->prepare("SELECT * FROM voiture WHERE  num_chassis =?");
  $reqnvnumchassisi->execute(array($nvnumchassis));
 $nvnumchassisiexist= $reqnvnumchassisi->rowCount();
 if($nvnumchassisiexist==0){
$req= $pdo->prepare("UPDATE  voiture SET num_chassis =? WHERE num_s=? ");
$req->execute(array($nvnumchassis,$voitureinfo['num_chassis ']));

  header("Location:../mes-déclarations.php?msg2=déclaration-modifiée&id=".$_SESSION['id']);  
 }else {
  $erreur="le numéro de chassis que vous avez rentré il existe déja";
 }
}
}
...
if(isset($_POST['nvclr']) and !empty($_POST['nvclr'] ) and $_POST['nvclr'] != $voitureinfo['couleur']){
  $nvclr=htmlspecialchars($_POST['nvclr']);
  $update=$pdo->prepare("UPDATE voiture SET couleur=? WHERE num_chassis= ?");
  $update->execute(array($nvclr,$getnum_chassis));
  header("Location:../mes-déclarations.php?msg2=déclaration-modifiée&id=".$_SESSION['id']);  
...
?>
      ...
      <h2>modifier votre déclaration de voiture <script src="https://cdn.lordicon.com/libs/mssddfmo/lord-icon-2.1.0.js"></script>
        <lord-icon
   ...
   <form name="formaadd" method="post" action="" enctype="multipart/form-data">
      ...
      <input type="text" name="nvmarque" id="nvmarque" placeholder="<?php echo $voitureinfo['marque'] ?>">
    </div>
    <div class="inputbx"data-center-center="opacity:1;left:0;"data-0-bottom="opacity:0;left:-50px;">
      <label>Nom de la voiture</label>
      <input type="text" name="nvnom" id="nvnom" placeholder="<?php echo $voitureinfo['nom_voiture'] ?>">
    </div>
    <div class="inputbx"data-center-center="opacity:1;left:0;"data-0-bottom="opacity:0;left:-50px;">
      <label> numéro de châssis </label> 
      <input type="text" name="nvnumchassis" id="nvnumchassis" placeholder="le numéro de chassis">
    </div>
    <div class="inputbx"data-center-center="opacity:1;left:0;"data-0-bottom="opacity:0;left:-50px;">
      <label>Maticule</label>
      <input type="text" name="nvmatricule" id="nvmatricule" placeholder="<?php echo $voitureinfo['matricule'] ?>">
    </div>
    <div class="inputbx"data-center-center="opacity:1;left:0;"data-0-bottom="opacity:0;left:-50px;">
      <label>Couleur de voiture</label>
      <input type="text" name="nvclr" id="nvclr" placeholder="<?php echo $voitureinfo['couleur'] ?> ">
    </div>
            <br>
