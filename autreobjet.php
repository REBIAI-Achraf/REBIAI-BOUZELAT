...
$justificatifs=$_FILES['just']['name'];
   ...
   and!empty($_POST['clr'])
    ...
    if($numslenth <3 AND $numslenth >50){
	...
	$req= $pdo->prepare("INSERT INTO objet(id,nature,num_s,couleur,photo,justificatif,confirmation) VALUES (?,?,?,?,?,?,?)");
	$req->execute(array($ID,$nature,$nums,$couleur,$_FILES['image']['name'],$_FILES['just']['name'],''));
	if(isset($req)){
move_uploaded_file($_FILES["image"]["tmp_name"],"../autreobjets-déclarés/$img");
move_uploaded_file($_FILES["just"]["tmp_name"],"../justificatifs-autre/$justificatifs");
...
<html>
   <head>
      ...
      <title>Déclaration d'un objet</title>  
...
.info { color:#fff;font-size:1.3em;font-weight:200;text-shadow:0 0 3px black; animation:op .8s;position:relative;top:150px;left:-220px;}
@keyframes op{
  0%{
     opacity:0;
   }
...
?>
      ...
      <h2>Remplissez les informations de votre Objet <script src="https://cdn.lordicon.com/libs/mssddfmo/lord-icon-2.1.0.js"></script>
        <lord-icon
 ...
 <div class="info"><p><img src="../img/White-in-Basic5.png" height="30px"/>malheuresement,vous ne pouvez pas déclaré n'importe quel objet<br> il doit au moins avoir un numéro de série ou un numéro unique qui l'identifie </p></div>
    
<form name="formaadd" method="post" action="" enctype="multipart/form-data">
      <br>
      <div class="inputbx">
      <label>Nature d'objet</label>
      <input type="text" name="nature" id="nature" placeholder="Entrer le nature de votre objet">
    </div>
    <div class="inputbx">
...
<label for="file-input2"><i class="fas fa-cloud-download-alt"></i><span class="c">Justificatif</span></label><br>
           	<sub><p> justificarif téléchargé: <span id="name2">aucun</span></p></sub> 
            <script>
