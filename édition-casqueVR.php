...
if(isset($_GET['nums'])){
	$getnum_s=$_GET['nums'];
	$reqlap=$pdo->prepare("SELECT * FROM casque_vr WHERE num_s= ?");
...
/* modification de la marque*/
  if(isset($_POST['nvmarque']) and !empty($_POST['nvmarque'] ) and $_POST['nvmarque'] != $lapinfo['marque']){
    ...
    $update=$pdo->prepare("UPDATE  casque_vr SET marque =? WHERE num_s= ?");
...
if(isset($_POST['nvnom']) and !empty($_POST['nvnom'] ) and $_POST['nvnom'] != $lapinfo['nom_appareil']){
  ...
  $update=$pdo->prepare("UPDATE casque_vr SET nom_appareil=? WHERE num_s= ?");
...
if(isset($_POST['nvnums']) and !empty($_POST['nvnums'] ) and sha1($_POST['nvnums'])!= $lapinfo['num_s']){
  ...
  $reqnum_s=$pdo->prepare("SELECT * FROM casque_vr WHERE  num_s=?");
  $reqnum_s->execute(array($nvnums));
 $num_sexist= $reqnum_s->rowCount();
 if($num_sexist==0){
$req= $pdo->prepare("UPDATE  casque_vr SET num_s=? WHERE num_s=? ");
...
if(isset($_POST['nvclr']) and !empty($_POST['nvclr'] ) and $_POST['nvclr'] != $lapinfo['couleur']){
  ...
  $update=$pdo->prepare("UPDATE casque_vr SET couleur=? WHERE num_s= ?");
...
?>
      ...
      <h2>modifier votre déclaration du Casque VR<script src="https://cdn.lordicon.com/libs/mssddfmo/lord-icon-2.1.0.js"></script>
        <lord-icon
