...
$justificatifs=$_FILES['just']['name'];
   ...
   and!empty($_POST['clr'])
    ...
    if($imageFileType != "jpg" && $imageFileType != "png" && $imageFileType != "jpeg"){
      ...
      $reqnums=$pdo->prepare("SELECT * FROM console WHERE  num_s=?");
      $reqnums->execute(array($num_s));
     $numsexist= $reqnums->rowCount();
     if($numsexist==0){
      $req= $pdo->prepare("INSERT INTO console(id,marque,nom_appareil,num_s,couleur,photo,justificatif,confirmation) VALUES (?,?,?,?,?,?,?,?)");
      $req->execute(array($ID,$marque,$nom_appareil,$num_s,$couleur,$_FILES['image']['name'],$_FILES['just']['name'],''));
	if($req){
move_uploaded_file($_FILES["image"]["tmp_name"],"../consoles-déclarés/$img");
move_uploaded_file($_FILES["just"]["tmp_name"],"../justificatifs-console/$justificatifs");
...
<body>
  ...
  <section class="banner">
    <header>
      <a href="console.php" class="re" ><i id ="icon"class="fas fa-arrow-alt-circle-left"></i> Retour</a>
     <div class="alerte">
...
?>
      ...
      <h2>Remplissez les informations de votre console <script src="https://cdn.lordicon.com/libs/mssddfmo/lord-icon-2.1.0.js"></script>
        <lord-icon
   ...
   <form name="formaadd" method="post" action="" enctype="multipart/form-data">  
      ...
      <input type="text" name="marque" id="marque" placeholder="Entrer la marque de l'apareil">
    </div>
    <div class="inputbx">
    ...
    <div class="inputbx">
      <label>Couleur de la console</label>
