...
$justificatifs=$_FILES['just']['name'];
   ...
   and!empty($_POST['clr'])
    ...
    if($imageFileType != "jpg" && $imageFileType != "png" && $imageFileType != "jpeg"){
      ...
      $reqnums=$pdo->prepare("SELECT * FROM velo WHERE  num_s=?");
      $reqnums->execute(array($num_s));
     $numsexist= $reqnums->rowCount();
     if($numsexist==0){
      $req= $pdo->prepare("INSERT INTO velo (id,marque,type_velo,num_s,couleur,photo,justificatif,confirmation) VALUES (?,?,?,?,?,?,?,?)");
      $req->execute(array($ID,$marque,$type,$num_s,$couleur,$_FILES['image']['name'],$_FILES['just']['name'],''));
	if($req){
move_uploaded_file($_FILES["image"]["tmp_name"],"../vélos-déclarés/$img");
move_uploaded_file($_FILES["just"]["tmp_name"],"../justificatifs-vélo/$justificatifs");
    $erreur2="votre déclaration a été envoyée !";
  }
    }
     else{
      $erreur="vélo déja déclaré ,numéro de série déja existe !";
     }  
  }
...
<body>
  ...
  <section class="banner">
    <header>
      <a href="vélo.php" class="re" ><i id ="icon"class="fas fa-arrow-alt-circle-left"></i> Retour</a>
     <div class="alerte">
...
?>
      ...
      <h2>Remplissez les informations de votre vélo <script src="https://cdn.lordicon.com/libs/mssddfmo/lord-icon-2.1.0.js"></script>
        <lord-icon
   ...
   <form name="formaadd" method="post" action="" enctype="multipart/form-data">
    ...
    <div class="inputbx">
      ...
      <input type="text" name="type" id="type" placeholder="VTT, VTC ,VAE ...">
    </div>
    <div class="inputbx">
