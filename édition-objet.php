...
if(isset($_GET['nums'])){
	$getnum_s=$_GET['nums'];
	$reqob=$pdo->prepare("SELECT * FROM objet WHERE num_s= ?");
	$reqob->execute(array($getnum_s));
	$obinfo=$reqob->fetch();
/* modification de la nature */
  if(isset($_POST['nvnature']) and !empty($_POST['nvnature'] ) and $_POST['nvnature'] != $obinfo['nature']){
    $nvnature=htmlspecialchars($_POST['nvnature']);
    $update=$pdo->prepare("UPDATE  objet SET nature =? WHERE num_s= ?");
    $update->execute(array($nvnature,$getnum_s));
...
if(isset($_POST['nvnums']) and !empty($_POST['nvnums'] ) and sha1($_POST['nvnums'])!= $obinfo['num_s']){
  $nvnums=sha1($_POST['nvnums']);
  $num_slenth=strlen($_POST['nvnums']);
  if($num_slenth > 25 ){
    $erreur="le numéro de serie que vous avez rentré est invalide";
}else{
  $reqnum_s=$pdo->prepare("SELECT * FROM objet WHERE  num_s=?");
  $reqnum_s->execute(array($nvnums));
 $num_sexist= $reqnum_s->rowCount();
 if($num_sexist==0){
$req= $pdo->prepare("UPDATE  objet SET num_s=? WHERE num_s=? ");
$req->execute(array($nvnums,$obinfo['num_s']));
...
if(isset($_POST['nvclr']) and !empty($_POST['nvclr'] ) and $_POST['nvclr'] != $obinfo['couleur']){
  $nvclr=htmlspecialchars($_POST['nvclr']);
  $update=$pdo->prepare("UPDATE objet SET couleur=? WHERE num_s= ?");
...
?>
      ...
      <h2>modifier votre déclaration d'objet <script src="https://cdn.lordicon.com/libs/mssddfmo/lord-icon-2.1.0.js"></script>
        <lord-icon
   ...
   <form name="formaadd" method="post" action="" enctype="multipart/form-data">
      ...
      <input type="text" name="nvnature" id="nature" placeholder="<?php echo $obinfo['nature'];?>">
    </div>
    <div class="inputbx">
    ...
    <div class="inputbx">
      ...
      <input type="text" name="nvclr" id="clr" placeholder="<?php echo $obinfo['couleur'];?>">
    </div>
            <br>
