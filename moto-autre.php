...
{   $marque=$_POST['marque'];
    $nom_moto =htmlspecialchars($_POST['nom']);
    $num_chassislenth=strlen($_POST['numchassis']);
	$couleur=$_POST['clr'];
   	$num_chassis=sha1($_POST['numchassis']);
  $matricule=$_POST['matricule'];
  $matriculelenth=strlen($_POST['matricule']);
...
$justificatifs=$_FILES['just']['name'];
	...
	if(!empty($_POST['nom'])
   and!empty($_POST['numchassis'])
   and!empty($_POST['clr'])
   and!empty($_POST['matricule'])
   ){	
    if($matriculelenth > 10){
        $erreur="le matricule que vous avez rentré est invalide";
    }
      else if($num_chassislenth != 17 ){
    ...
    if($imageFileType != "jpg" && $imageFileType != "png" && $imageFileType != "jpeg"){
      ...
      $reqnumchassis=$pdo->prepare("SELECT * FROM moto WHERE num_chassis = ?");
      $reqnumchassis->execute(array($num_chassis));
     $numchassiexist= $reqnumchassis->rowCount();
     if($numchassiexist==0){
      $req= $pdo->prepare("INSERT INTO moto (id,marque,nom_moto,num_chassis,couleur,matricule,photo,justificatif,confirmation) VALUES (?,?,?,?,?,?,?,?,?)");
      $req->execute(array($ID,$marque,$nom_moto ,$num_chassis,$couleur, $matricule,$_FILES['image']['name'],$_FILES['just']['name'],''));
	if(isset($req)){
move_uploaded_file($_FILES["image"]["tmp_name"],"../motos-déclarés/$img");
move_uploaded_file($_FILES["just"]["tmp_name"],"../justificatifs-moto/$justificatifs");
    $erreur2="votre déclaration a été envoyée !";
  }
    }
     else{
      $erreur="moto déja déclaré , numéro de châssis existe déja !";
     }  
  }
...
<html>
   <head>
      ...
      <title>Déclaration de moto</title>  
...
<body>
  ...
  <section class="banner">
    <header>
      <a href="moto.php" class="re" ><i id ="icon"class="fas fa-arrow-alt-circle-left"></i> Retour</a>
     <div class="alerte">
...
?>
      ...
      <h2>Remplissez les informations de votre moto<script src="https://cdn.lordicon.com/libs/mssddfmo/lord-icon-2.1.0.js"></script>
        <lord-icon
   ...
   <form name="formaadd" method="post" action="" enctype="multipart/form-data">
      ...
      <input type="text" name="marque" id="marque" placeholder="Entrer le nom de l'apareil">
    </div>
    <div class="inputbx"data-center-center="opacity:1;left:0;"data-0-bottom="opacity:0;left:-50px;">
    ...
    <div class="inputbx"data-center-center="opacity:1;left:0;"data-0-bottom="opacity:0;left:-50px;">
      ...
      <input type="text" name="numchassis" id="numchassis" placeholder="Entrer le numéro de chassis">
    </div>
    <div class="inputbx"data-center-center="opacity:1;left:0;"data-0-bottom="opacity:0;left:-50px;">
      <label>Maticule</label>
      <input type="text" name="matricule" id="matricule" placeholder="Entrer le Matricule">
    </div>
    <div class="inputbx"data-center-center="opacity:1;left:0;"data-0-bottom="opacity:0;left:-50px;">
