...
if(isset($_GET['nums'])){
	$getnum_s=$_GET['nums'];
	$reqconsole=$pdo->prepare("SELECT * FROM console WHERE num_s= ?");
	$reqconsole->execute(array($getnum_s));
	$consoleinfo=$reqconsole->fetch();
/* modification de la marque*/
  if(isset($_POST['nvmarque']) and !empty($_POST['nvmarque'] ) and $_POST['nvmarque'] != $consoleinfo['marque']){
    $nvmarque=htmlspecialchars($_POST['nvmarque']);
    $update=$pdo->prepare("UPDATE  console SET marque =? WHERE num_s= ?");
    $update->execute(array($nvmarque,$getnum_s));
    header("Location:../mes-déclarations.php?msg2=déclaration-modifiée&id=".$_SESSION['id']);  
   
}
/* modification de type console */
if(isset($_POST['nvnom']) and !empty($_POST['nvnom'] ) and $_POST['nvnom'] != $consoleinfo['nom_appareil']){
  $nvnom=htmlspecialchars($_POST['nvnom']);
  $update=$pdo->prepare("UPDATE console SET nom_appareil=? WHERE num_s= ?");
...
if(isset($_POST['nvnums']) and !empty($_POST['nvnums'] ) and sha1($_POST['nvnums'])!= $consoleinfo['num_s']){
  $nvnums=sha1($_POST['nvnums']);
  $num_slenth=strlen($_POST['nvnums']);
  if($num_slenth > 25 ){
    $erreur="le numéro de serie que vous avez rentré est invalide";
}else{
  $reqnum_s=$pdo->prepare("SELECT * FROM console WHERE  num_s=?");
  $reqnum_s->execute(array($nvnums));
 $num_sexist= $reqnum_s->rowCount();
 if($num_sexist==0){
$req= $pdo->prepare("UPDATE  console SET num_s=? WHERE num_s=? ");
$req->execute(array($nvnums,$consoleinfo['num_s']));
...
if(isset($_POST['nvclr']) and !empty($_POST['nvclr'] ) and $_POST['nvclr'] != $consoleinfo['couleur']){
  $nvclr=htmlspecialchars($_POST['nvclr']);
  $update=$pdo->prepare("UPDATE console SET couleur=? WHERE num_s= ?");
...
?>
      ...
      <h2>modifier votre déclaration de console <script src="https://cdn.lordicon.com/libs/mssddfmo/lord-icon-2.1.0.js"></script>
        <lord-icon
   ...
   <form name="formaadd" method="post" action="" enctype="multipart/form-data">
      ...
      <br>
      <div class="inputbx">
      <label>Marque de la console</label>
      <input type="text" name="nvmarque" id="marque" placeholder="<?php echo $consoleinfo['marque'];?>">
    </div>
    <div class="inputbx">
      <label>Nom /modèle de la console</label>
      <input type="text" name="nvnom" id="nvnom" placeholder="<?php echo $consoleinfo['nom_appareil'];?>">
    </div>
    <div class="inputbx">
    ...
    <div class="inputbx">
      <label>Couleur de console </label>
      <input type="text" name="nvclr" id="clr" placeholder="<?php echo $consoleinfo['couleur'];?>">
    </div>
            <br>
