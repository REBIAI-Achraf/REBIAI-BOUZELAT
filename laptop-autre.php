...
$justificatifs=$_FILES['just']['name'];
   ...
   and!empty($_POST['xemei'])
    ...
    if($imageFileType != "jpg" && $imageFileType != "png" && $imageFileType != "jpeg"){
      ...
      $reqnums=$pdo->prepare("SELECT * FROM Laptop WHERE  num_s=?");
      $reqnums->execute(array($num_s));
     $numsexist= $reqnums->rowCount();
     if($numsexist==0){
      $req= $pdo->prepare("INSERT INTO Laptop(id,marque,nom_appareil,num_s,couleur,id_modele,photo,justificatif,confirmation) VALUES (?,?,?,?,?,?,?,?,?)");
      $req->execute(array($ID,$marque,$nom_appareil,$num_s,$couleur,$idm,$_FILES['image']['name'],$_FILES['just']['name'],''));
	if($req){
move_uploaded_file($_FILES["image"]["tmp_name"],"../laptops-déclarés/$img");
move_uploaded_file($_FILES["just"]["tmp_name"],"../justificatifs-lap/$justificatifs");
...
<html>
   <head>
      ...
      <title>Déclaration de Laptop</title>  
...
<body>
  ...
  <section class="banner">
    <header>
      <a href="laptop.php" class="re" ><i id ="icon"class="fas fa-arrow-alt-circle-left"></i> Retour</a>
     <div class="alerte">
...
?>
      ...
      <h2>Remplissez les informations de votre Laptop <script src="https://cdn.lordicon.com/libs/mssddfmo/lord-icon-2.1.0.js"></script>
        <lord-icon
   ...
   <form name="formaadd" method="post" action="" enctype="multipart/form-data">
    ...
    <div class="inputbx">
      ...
      <input type="text" name="idm" id="idm" placeholder="Entrer l'id du modèle ">
    </div>
    <div class="inputbx">
