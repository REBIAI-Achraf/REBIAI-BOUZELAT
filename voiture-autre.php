...
{   $marque=$_POST['marque'];
    $nom_voiture =htmlspecialchars($_POST['nom']);
...
$justificatifs=$_FILES['just']['name'];
   ...
   and!empty($_POST['matricule'])
    ...
    if($imageFileType != "jpg" && $imageFileType != "png" && $imageFileType != "jpeg"){
      ...
      $reqnumchassis=$pdo->prepare("SELECT * FROM voiture WHERE num_chassis = ?");
      $reqnumchassis->execute(array($num_chassis));
     $numchassiexist= $reqnumchassis->rowCount();
     if($numchassiexist==0){
      $req= $pdo->prepare("INSERT INTO voiture (id,marque,nom_voiture,num_chassis,couleur,matricule,photo,justificatif,confirmation) VALUES (?,?,?,?,?,?,?,?,?)");
      $req->execute(array($ID,$marque,$nom_voiture ,$num_chassis,$couleur, $matricule,$_FILES['image']['name'],$_FILES['just']['name'],''));
	if(isset($req)){
move_uploaded_file($_FILES["image"]["tmp_name"],"../voitures-déclarés/$img");
move_uploaded_file($_FILES["just"]["tmp_name"],"../justificatifs-voiture/$justificatifs");
    $erreur2="votre déclaration a été envoyée !";
  }
    }
     else{
      $erreur="voiture déja déclaré , numéro de châssis existe déja !";
     }  
  }
...
<body>
  ...
  <section class="banner">
    <header>
      <a href="voiture.php" class="re" ><i id ="icon"class="fas fa-arrow-alt-circle-left"></i> Retour</a>
     <div class="alerte">
...
?>
      ...
      <h2>Remplissez les informations de votre voiture<script src="https://cdn.lordicon.com/libs/mssddfmo/lord-icon-2.1.0.js"></script>
        <lord-icon
   ...
   <form name="formaadd" method="post" action="" enctype="multipart/form-data">
      ...
      <input type="text" name="marque" id="marque" placeholder="Entrer le nom de la voiture">
    </div>
    <div class="inputbx"data-center-center="opacity:1;left:0;"data-0-bottom="opacity:0;left:-50px;">
