...
if(isset($_GET['nums'])){
	$getnum_s=$_GET['nums'];
	$reqtv=$pdo->prepare("SELECT * FROM tv WHERE num_s= ?");
	$reqtv->execute(array($getnum_s));
	$tvinfo=$reqtv->fetch();
/* modification de la marque*/
  if(isset($_POST['nvmarque']) and !empty($_POST['nvmarque'] ) and $_POST['nvmarque'] != $tvinfo['marque']){
    $nvmarque=htmlspecialchars($_POST['nvmarque']);
    $update=$pdo->prepare("UPDATE  tv SET marque =? WHERE num_s= ?");
    $update->execute(array($nvmarque,$getnum_s));
    header("Location:../mes-déclarations.php?msg2=déclaration-modifiée&id=".$_SESSION['id']);  
   
}
/* modification de type tv */
if(isset($_POST['typetv']) and !empty($_POST['typetv'] ) and $_POST['typetv'] != $tvinfo['nom_tvareil']){
  $typetv=htmlspecialchars($_POST['typetv']);
  $update=$pdo->prepare("UPDATE tv SET typeTV=? WHERE num_s= ?");
  $update->execute(array($typetv,$getnum_s));
  header("Location:../mes-déclarations.php?msg2=déclaration-modifiée&id=".$_SESSION['id']);  
 
}

/* modification de num serie */
if(isset($_POST['nvnums']) and !empty($_POST['nvnums'] ) and sha1($_POST['nvnums'])!= $tvinfo['num_s']){
  $nvnums=sha1($_POST['nvnums']);
  $num_slenth=strlen($_POST['nvnums']);
  if($num_slenth > 25 ){
    $erreur="le numéro de serie que vous avez rentré est invalide";
}else{
  $reqnum_s=$pdo->prepare("SELECT * FROM tv WHERE  num_s=?");
  $reqnum_s->execute(array($nvnums));
 $num_sexist= $reqnum_s->rowCount();
 if($num_sexist==0){
$req= $pdo->prepare("UPDATE  tv SET num_s=? WHERE num_s=? ");
$req->execute(array($nvnums,$tvinfo['num_s']));
...
if(isset($_POST['nvclr']) and !empty($_POST['nvclr'] ) and $_POST['nvclr'] != $tvinfo['couleur']){
  $nvclr=htmlspecialchars($_POST['nvclr']);
  $update=$pdo->prepare("UPDATE tv SET couleur=? WHERE num_s= ?");
  $update->execute(array($nvclr,$getnum_s));
  header("Location:../mes-déclarations.php?msg2=déclaration-modifiée&id=".$_SESSION['id']);  
 
}  
 /* modification de taille */
 if(isset($_POST['nvtaille']) and !empty($_POST['nvtaille'] ) and $_POST['nvtaille'] != $tvinfo['couleur']){
    $nvtaille=htmlspecialchars($_POST['nvtaille']);
    $update=$pdo->prepare("UPDATE tv SET taille=? WHERE num_s= ?");
    $update->execute(array($nvtaille,$getnum_s));
    header("Location:../mes-déclarations.php?msg2=déclaration-modifiée&id=".$_SESSION['id']);  
...
?>
      ...
      <h2>modifier votre déclaration de TV <script src="https://cdn.lordicon.com/libs/mssddfmo/lord-icon-2.1.0.js"></script>
        <lord-icon
   ...
   <form name="formaadd" method="post" action="" enctype="multipart/form-data">
      ...
      <br>
      <div class="inputbx">
      <label>Marque de TV</label>
      <input type="text" name="nvmarque" id="marque" placeholder="<?php echo $tvinfo['marque'];?>">
    </div>
    <div class="inputbx">
      <label>Type de TV</label>
      <input type="text" name="typetv" id="nom" placeholder="<?php echo $tvinfo['typeTV'];?>">
    </div>
    <div class="inputbx">
    ...
    <div class="inputbx">
      <label>Couleur de TV </label>
      <input type="text" name="nvclr" id="clr" placeholder="<?php echo $tvinfo['couleur'];?>">
    </div>
    <div class="inputbx">
      <label>Taille de TV</label>
      <input type="text" name="nvtaille" id="nvtaille" placeholder="<?php echo $tvinfo['taille'];?>">
    </div>
            <br>
