...
if(isset($_GET['imei'])){
	$getimei=$_GET['imei'];
	$reqsm=$pdo->prepare("SELECT * FROM smart_watch WHERE imei= ?");
	$reqsm->execute(array($getimei));
	$sminfo=$reqsm->fetch();
/* modification de la marque*/
  if(isset($_POST['nvmarque']) and !empty($_POST['nvmarque'] ) and $_POST['nvmarque'] != $sminfo['marque']){
    $nvmarque=htmlspecialchars($_POST['nvmarque']);
    $update=$pdo->prepare("UPDATE  smart_watch SET marque =? WHERE imei= ?");
    $update->execute(array($nvmarque,$getimei));
    header("Location:../mes-déclarations.php?msg2=déclaration-modifiée&id=".$_SESSION['id']);  
   
}
/* modification de nom */
if(isset($_POST['nvnom']) and !empty($_POST['nvnom'] ) and $_POST['nvnom'] != $sminfo['nom_appareil']){
  $nvnom=htmlspecialchars($_POST['nvnom']);
  $update=$pdo->prepare("UPDATE smart_watch SET nom_appareil=? WHERE imei= ?");
  $update->execute(array($nvnom,$getimei));
  header("Location:../mes-déclarations.php?msg2=déclaration-modifiée&id=".$_SESSION['id']);  
 
}
/* modification de num serie */
if(isset($_POST['nvnums']) and !empty($_POST['nvnums'] ) and $_POST['nvnums'] != $sminfo['imei']){
  $nvnums=htmlspecialchars($_POST['nvnums']);
  $update=$pdo->prepare("UPDATE smart_watch SET imei=? WHERE imei= ?");
  $update->execute(array($nvnums,$getimei));
  header("Location:../mes-déclarations.php?msg2=déclaration-modifiée&id=".$_SESSION['id']);  
 
}
/* modification de 'imei */
if(isset($_POST['nvximei']) and !empty($_POST['nvximei'] ) and sha1($_POST['nvximei'])!= $sminfo['imei']){
  $nvximei=sha1($_POST['nvximei']);
  $imeilenth=strlen($_POST['nvximei']);
  if($imeilenth < 15 or $imeilenth > 18 ){
    $erreur="l'Imei que vous avez rentré est invalide";
}else{
  $reqimei=$pdo->prepare("SELECT * FROM smart_watch WHERE  imei=?");
  $reqimei->execute(array($nvximei));
 $imeiexist= $reqimei->rowCount();
 if($imeiexist==0){
$req= $pdo->prepare("UPDATE  smart_watch SET imei=? WHERE imei=? ");
$req->execute(array($nvximei,$sminfo['imei']));
...
if(isset($_POST['nvclr']) and !empty($_POST['nvclr'] ) and $_POST['nvclr'] != $sminfo['couleur']){
  $nvclr=htmlspecialchars($_POST['nvclr']);
  $update=$pdo->prepare("UPDATE smart_watch SET couleur=? WHERE imei= ?");
  $update->execute(array($nvclr,$getimei));
  header("Location:../mes-déclarations.php?msg2=déclaration-modifiée&id=".$_SESSION['id']);  
...
?>
      ...
      <h2>modifier votre déclaration de Smart watch<script src="https://cdn.lordicon.com/libs/mssddfmo/lord-icon-2.1.0.js"></script>
        <lord-icon
   ...
   <form name="formaadd" method="post" action="" enctype="multipart/form-data">
      ...
      <input type="text" name="nvmarque" id="nvmarque" placeholder="<?php echo $sminfo['marque'];?>">
    </div>
    <div class="inputbx"data-center-center="opacity:1;left:0;"data-0-bottom="opacity:0;left:-50px;">
      <label>Nom de l'appareil</label>
      <input type="text" name="nvnom" id="nvnom" placeholder="<?php echo $sminfo['nom_appareil'];?>">
    </div>
    <div class="inputbx"data-center-center="opacity:1;left:0;"data-0-bottom="opacity:0;left:-50px;">
      <label>Imei</label>
      <input type="text" name="nvximei" id="nvximei" placeholder="l'IMEI">
    </div>
    <div class="inputbx"data-center-center="opacity:1;left:0;"data-0-bottom="opacity:0;left:-50px;">
      <label>Couleur de l'appareil</label>
      <input type="text" name="nvclr" id="nvclr" placeholder="<?php echo $sminfo['couleur'];?>">
    </div>
            <br>
