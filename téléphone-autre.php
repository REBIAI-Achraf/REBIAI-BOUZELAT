...
$justificatifs=$_FILES['just']['name'];
   ...
   and!empty($_POST['xemei'])
    ...
    if($imageFileType != "jpg" && $imageFileType != "png" && $imageFileType != "jpeg"){
     ...
     if($emeiexist==0){
	$req= $pdo->prepare("INSERT INTO telephone(id,marque,nom_appareil,num_s,couleur,emei,photo,justificatif,confirmation) VALUES (?,?,?,?,?,?,?,?,?)");
	$req->execute(array($ID,$marque,$nom_appareil,$num_s,$couleur,$emei,$_FILES['image']['name'],$_FILES['just']['name'],''));
	if($req){
move_uploaded_file($_FILES["image"]["tmp_name"],"../téléphones-déclarés/$img");
move_uploaded_file($_FILES["just"]["tmp_name"],"../justificatifs-tel/$justificatifs");
    $erreur2="votre déclaration a été envoyée !";
  }
    }
     else{
      $erreur="appareil déja déclaré , emei existe déja !";
     }  
  }
...
<body>
  ...
  <section class="banner">
    <header>
      <a href="telephone.php" class="re" ><i id ="icon"class="fas fa-arrow-alt-circle-left"></i> Retour</a>
     <div class="alerte">
...
?>
      ...
      <h2>Remplissez les informations de votre téléphone <script src="https://cdn.lordicon.com/libs/mssddfmo/lord-icon-2.1.0.js"></script>
        <lord-icon
